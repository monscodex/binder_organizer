...
class BinderOrganizer:
    ...
    def try_read_config_file(self, config_file_path: str) -> None:
    ...
    def read_paper_weight_per_assignment(self) -> Dict[str, float]:
        ...
        if paper_weight_per_assignment == {}:
            ...
            paper_weight_per_assignment = {
                assignment.upper(): self.weight_per_height_unit * float(width) for assignment, width in paper_width_section.items()
            }

        return paper_weight_per_assignment
    
    def get_absolute_best_combination(self) -> Dict[str, Union[list, float]]:
        ...
        while True:
            ...
            if [] in current_best_combination['assignments_combination']:
                prior_best_combination['number_of_binders'] = n - 1
                return prior_best_combination
            
            prior_best_combination = current_best_combination
            n += 1

    def get_best_combination_for_n_binders(self, number_of_binders : int, give_worse_combination: bool=False) -> Dict[str, Union[int, float]]:
        best_combination  = {
            'average_carried_weight': 100000000000000000,
        }

        worse_combination = {
            'average_carried_weight': 0,
        }
        ...
        for assignments_combination in get_partitions_of_list(self.binder_assignments, number_of_binders):
            # Average carried weight with this combination according to the schedule in the config file
            average_carried_weight = self.get_average_carried_weight_with_assignments_combination(assignments_combination)

            best_combination, worse_combination = self.change_worse_or_best_combination_if_needed(
        ...
        if give_worse_combination:
            return {'best': best_combination, 'worse': worse_combination}
        
        return best_combination

    
    def try_read_specified_weight_per_assignment(self) -> Dict:
        ...
        paper_weight_per_assignment = {
            assignment.upper(): float(weight) for assignment, weight in paper_weight_section.items()
        }
    ...
    def read_schedule(self) -> list:
        ...
        while True:
            ...
            week = [day_schedule.split(' ')
                    for day, day_schedule in dict(section).items()]
            ...
            # assignments (str))
            week = [[before_switch.split(',')
                     for before_switch in day] for day in week]
    ...
    def change_worse_or_best_combination_if_needed(self, assignments_combination: list, average_carried_weight: Union[float, int], best_combination : Dict, worse_combination: Dict) -> (Dict, Dict):
    ...
    def get_average_carried_weight_with_assignments_combination(self, assignments_combination : list) -> Union[int, float]:
        ...
        binders = self.get_binders_out_of_assignments_combination(assignments_combination)
        ...
        for week in self.schedule:
            for day in week:
                for assignments_before_switch in day:
                    carried_weight = self.get_carried_weight_before_switch(binders, assignments_before_switch)

                    carried_weights_before_switches.append(carried_weight)


        average_carried_weight = sum(carried_weights_before_switches) / len(carried_weights_before_switches)
        
        return average_carried_weight
    

    def get_carried_weight_before_switch(self, binders: list, assignments_before_switch: list) -> Union[int, float]:
        ...
        total_binders_weight = self.get_weight_of_binders_with_indexes(binders_needed, binders)

        return total_binders_weight

    def add_binder_index_if_needed(self, binders_needed : Set, assignment : str, binders: list) -> None:
        binder_needed = self.get_index_of_binder_containing_assignment(assignment, binders)
    ...
    def get_binders_out_of_assignments_combination(self, assignments_combination: list) -> list:
        binders = [{ 'assignments': binder } for binder in assignments_combination]
    ...
    def read_empty_binder_weight(self) -> Union[int, float]:
        section = self.config['GENERAL']

        return float(section['EMPTY_BINDER_WEIGHT'])


    def read_binder_assignments(self) -> list:
        assignments = list(self.weight_per_assignment.keys())

        return assignments


    def get_index_of_binder_containing_assignment(self, assignment: str, binders: list) -> int:
        index = None
    ...
    def get_weight_of_binders_with_indexes(self, binders_indexes: set, binders: list) -> Union[int, float]:
        weight = 0
